 ...
 */
float computeFresnel(vec3 normal)
{
	vec3 inv_dir = normalize(u_cameraPosition.xyz - v_pos);
	return clamp(dot(inv_dir, normal), 0.0, 1.0);
}
...
void main() {
	...
	gl_FragColor.a = pow(computeFresnel(normal), 2.5);
}
