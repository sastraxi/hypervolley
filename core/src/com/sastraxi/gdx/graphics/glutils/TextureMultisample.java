 ...
 * <p>
 * A Texture has to be bound via the {@link TextureMultisample#bind()} method in order for it to be applied to geometry. The texture will be
...
public class TextureMultisample extends GLTexture {
	...
	public TextureMultisample(String internalPath) {
		this(Gdx.files.internal(internalPath));
	}

	public TextureMultisample(FileHandle file) {
		this(file, null, false);
	}

	public TextureMultisample(FileHandle file, boolean useMipMaps) {
		this(file, null, useMipMaps);
	}

	public TextureMultisample(FileHandle file, Format format, boolean useMipMaps) {
		this(TextureData.Factory.loadFromFile(file, format, useMipMaps), false);
	}

	public TextureMultisample(Pixmap pixmap) {
		this(new PixmapTextureData(pixmap, null, false, false), false);
	}

	public TextureMultisample(Pixmap pixmap, boolean useMipMaps) {
		this(new PixmapTextureData(pixmap, null, useMipMaps, false), false);
	}

	public TextureMultisample(Pixmap pixmap, Format format, boolean useMipMaps) {
		this(new PixmapTextureData(pixmap, format, useMipMaps, false), false);
	}

	public TextureMultisample(int width, int height, Format format) {
		this(new PixmapTextureData(new Pixmap(width, height, format), null, false, true), false);
	}

	public TextureMultisample(TextureData data, boolean multisample) {
		super(multisample ? GL32.GL_TEXTURE_2D_MULTISAMPLE : GL20.GL_TEXTURE_2D, Gdx.gl.glGenTexture());
		load(data);
		if (data.isManaged()) throw new GdxRuntimeException("TextureMultisample does not support managed texture data");
	}
	...
	public void load (TextureData data) {
		...
		bind();
		uploadImageData(this.glTarget, data);
